	...
	public void run() {
		...
		try {
			...
			while (kraj == false) {
				...
				clientOutput.println("2) Rezerviši kartu.");
				...
				case "2)": {
					...
					if (brojKarata - pom < 0) {
						...
						clientOutput.println("Uspešno ste kupili: " + pom + " karata.");
				...
				case "3)": {
					...
					try (FileWriter fw = new FileWriter("korisnici.txt", true);
						...
						System.out.println(
								"Greška prilikom pravljenja ili dodavanja u tekstualni fajl: " + e.getMessage());
					}
					break;
				}
				case "4)": {
